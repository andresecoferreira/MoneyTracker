<template>
	...
	<qtable 
		...
		<template #MNT="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'MNT' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					MNT : { active: true, Cod: "MNT", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.MNT.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.MNT.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.MNT.Description),
					name: "MNT",
					slot_name: "MNT",
					sort: false,
					visibility: vm.Modules.MNT.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {

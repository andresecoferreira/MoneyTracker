...
namespace CSGenio.business
{
	...
	/// Expense
	/// </summary>
	public class CSGenioAexpense : DbArea
	{
		/// <summary>
		...
		public CSGenioAexpense(User user, string module)
		{
            this.user = user;
            this.module = module;
			// USE /[MANUAL MNT CONSTRUTOR EXPENSE]/
		}

		public CSGenioAexpense(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codexpense", FieldType.KEY_INT);
			...
			Qfield.Decimals = 2;
			Qfield.CavDesignation = "VALUE__44420";
		...
		public static FieldRef FldCodexpense { get { return m_fldCodexpense; } }
		private static FieldRef m_fldCodexpense = new FieldRef("expense", "codexpense");

		/// <summary>Field : "" Tipo: "+" Formula:  ""</summary>
		public string ValCodexpense
		{
		...
		private static FieldRef m_fldGroup_id = new FieldRef("expense", "group_id");
        ...
        public static CSGenioAexpense search(PersistentSupport sp, string key, User user, string[] fields = null, bool forUpdate = false)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAexpense area = new CSGenioAexpense(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAexpense> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAexpense>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAexpense> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAexpense>(where, listing);
        }

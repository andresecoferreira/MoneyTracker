...
namespace GenioMVC.Controllers
{
	public partial class InvestmentController : ControllerBase
	{
		...
		public InvestmentController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION INVESTMENT]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAinvestment>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER INVESTMENT]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Investment row = new Models.Investment(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("investment");
					...
					case "INVESTMENT__CATEGORY__NAME":	// Field (DB)
						{
							...
							model.Load_Investment__category__name(qs);
							result = model.TableCategoryName;
						}
						break;
					case "INVESTMENT__MEMBER__NAME":	// Field (DB)
						{
							var model = new Investment_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Investment__member__name(qs);
							result = model.TableMemberName;
						}
						break;
					case "INVESTMENT__SOURCE__TITLE":	// Field (DB)
						{
							var model = new Investment_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Investment__source__title(qs);
			...
			try
			{
				...
				switch (string.IsNullOrEmpty(Identifier) ? "" : Identifier)
				{
					case "INVESTMENT__CATEGORY__NAME":	// Field (DB)
						...
						break;
					case "INVESTMENT__MEMBER__NAME":	// Field (DB)
						values = new Investment_ViewModel(UserContext.Current).GetDependant_InvestmentTableMemberName(Selected);
						break;
					case "INVESTMENT__SOURCE__TITLE":	// Field (DB)
						values = new Investment_ViewModel(UserContext.Current).GetDependant_InvestmentTableSourceTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Investment" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Investment([FromBody]Investment_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "investment",
				(primaryKey) => Models.Investment.Find(primaryKey, UserContext.Current, "FINVESTMENT"),
				(model) => formData.MapToModel(model as Models.Investment)
			);
		}
		...
		{
			return base.GetDocumsTickets("INVESTMENT", requestModel.FieldName, requestModel.KeyValue);
		}

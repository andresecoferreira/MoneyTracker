...
namespace GenioMVC.Controllers
{
	public partial class MonthController : ControllerBase
	{
		...
		public MonthController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION MONTH]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAmonth>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER MONTH]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Month row = new Models.Month(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("month");
			...
			try
			{
				...
				{
					case "MONTH__YEAR__YEAR_NUMBER":	// Field (DB)
						{
							var model = new Month_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Month__year__year_number(qs);
							result = model.TableYearYear_number;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "MONTH__YEAR__YEAR_NUMBER":	// Field (DB)
						values = new Month_ViewModel(UserContext.Current).GetDependant_MonthTableYearYear_number(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Month" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Month([FromBody]Month_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "month",
				(primaryKey) => Models.Month.Find(primaryKey, UserContext.Current, "FMONTH"),
				(model) => formData.MapToModel(model as Models.Month)
			);
		}
		...
		{
			return base.GetDocumsTickets("MONTH", requestModel.FieldName, requestModel.KeyValue);
		}

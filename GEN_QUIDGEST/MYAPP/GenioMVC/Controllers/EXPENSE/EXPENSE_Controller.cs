...
namespace GenioMVC.Controllers
{
	public partial class ExpenseController : ControllerBase
	{
		...
		public ExpenseController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION EXPENSE]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAexpense>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER EXPENSE]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Expense row = new Models.Expense(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("expense");
			...
			try
			{
				...
				{
					case "EXPENSE__CATEGORY_TYPE__NAME":	// Field (DB)
						{
							var model = new Expense_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Expense__category_type__name(qs);
							result = model.TableCategory_typeName;
						}
						break;
					case "EXPENSE__CATEGORY__NAME":	// Field (DB)
						{
							var model = new Expense_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Expense__category__name(qs);
							result = model.TableCategoryName;
						}
						break;
					case "EXPENSE__MEMBER__NAME":	// Field (DB)
						{
							var model = new Expense_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Expense__member__name(qs);
							result = model.TableMemberName;
						}
						break;
					case "EXPENSE__SOURCE__TITLE":	// Field (DB)
						{
							var model = new Expense_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Expense__source__title(qs);
							result = model.TableSourceTitle;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "EXPENSE__CATEGORY_TYPE__NAME":	// Field (DB)
						values = new Expense_ViewModel(UserContext.Current).GetDependant_ExpenseTableCategory_typeName(Selected);
						break;
					case "EXPENSE__CATEGORY__NAME":	// Field (DB)
						values = new Expense_ViewModel(UserContext.Current).GetDependant_ExpenseTableCategoryName(Selected);
						break;
					case "EXPENSE__MEMBER__NAME":	// Field (DB)
						values = new Expense_ViewModel(UserContext.Current).GetDependant_ExpenseTableMemberName(Selected);
						break;
					case "EXPENSE__SOURCE__TITLE":	// Field (DB)
						values = new Expense_ViewModel(UserContext.Current).GetDependant_ExpenseTableSourceTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Expense" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Expense([FromBody]Expense_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "expense",
				(primaryKey) => Models.Expense.Find(primaryKey, UserContext.Current, "FEXPENSE"),
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Wd_last_expenses([FromBody]Wd_last_expenses_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "expense",
				(primaryKey) => Models.Expense.Find(primaryKey, UserContext.Current, "FWD_LAST_EXPENSES"),
				(model) => formData.MapToModel(model as Models.Expense)
			);
		}
		...
		{
			return base.GetDocumsTickets("EXPENSE", requestModel.FieldName, requestModel.KeyValue);
		}

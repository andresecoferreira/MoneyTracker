...
namespace GenioMVC.Controllers
{
	...
	{
		private static readonly NavigationLocation ACTION_MNT_MENU_121 = new NavigationLocation("CATEGORY_TYPES63920", "MNT_Menu_121", "Category_type") { vueRouteName = "menu-MNT_121" };


		//
		// GET: /Category_type/MNT_Menu_121
		[ActionName("MNT_Menu_121")]
		[HttpPost]
		public ActionResult MNT_Menu_121([FromBody] RequestMenuModel requestModel)
		{
			var queryParams = requestModel.QueryParams;

			MNT_Menu_121_ViewModel model = new(m_userContext);
			...
			if (isHomePage)
				Navigation.SetValue("HomePage", "MNT_Menu_121");
			...
			if (!isHomePage &&
				(Navigation.CurrentLevel == null || !ACTION_MNT_MENU_121.IsSameAction(Navigation.CurrentLevel.Location)) &&
				Navigation.CurrentLevel.Location.Action != ACTION_MNT_MENU_121.Action)
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			else if (isHomePage)
			{
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_MNT_MENU_121.ShortDescription());
				Navigation.SetValue("HomePageContainsList", true);
			}



// USE /[MANUAL MNT MENU_GET 121]/
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "MNT_Menu_121_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_MNT_MENU_121.Name);
		...
		//
		// POST: /Category_type/MNT_Menu_121_UploadFile
		[HttpPost]
		public ActionResult MNT_Menu_121_UploadFile(string importType, string qqfile) {
			MNT_Menu_121_ViewModel model = new MNT_Menu_121_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAcategory_type> rows = new List<CSGenioAcategory_type>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAcategory_type>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAcategory_type importRow in rows)
				{
					try
					{
						lineNumber++;

...
namespace GenioMVC.Controllers
{
	public partial class IncomeController : ControllerBase
	{
		...
		public IncomeController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION INCOME]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAincome>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER INCOME]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Income row = new Models.Income(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("income");
					...
					case "INCOME__CATEGORY__NAME":	// Field (DB)
						{
							var model = new Income_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Income__category__name(qs);
							result = model.TableCategoryName;
						}
						break;
					case "INCOME__MEMBER__NAME":	// Field (DB)
						{
							var model = new Income_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Income__member__name(qs);
							result = model.TableMemberName;
						}
						break;
					case "INCOME__SOURCE__TITLE":	// Field (DB)
						{
							var model = new Income_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Income__source__title(qs);
			...
			try
			{
				...
				switch (string.IsNullOrEmpty(Identifier) ? "" : Identifier)
				{
					...
					case "INCOME__CATEGORY__NAME":	// Field (DB)
						...
						break;
					case "INCOME__MEMBER__NAME":	// Field (DB)
						values = new Income_ViewModel(UserContext.Current).GetDependant_IncomeTableMemberName(Selected);
						break;
					case "INCOME__SOURCE__TITLE":	// Field (DB)
						values = new Income_ViewModel(UserContext.Current).GetDependant_IncomeTableSourceTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Income" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Income([FromBody]Income_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "income",
				(primaryKey) => Models.Income.Find(primaryKey, UserContext.Current, "FINCOME"),
				(model) => formData.MapToModel(model as Models.Income)
			);
		}
		...
		{
			return base.GetDocumsTickets("INCOME", requestModel.FieldName, requestModel.KeyValue);
		}

...
{
	public class Wd_expenses_ViewModel : FormViewModel<Models.Expense>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Wd_expenses_ViewModel() : base(null!) { }

		public Wd_expenses_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FWD_EXPENSES", nestedForm) { }

		public Wd_expenses_ViewModel(UserContext userContext, Models.Expense row, bool nestedForm = false) : base(userContext, "FWD_EXPENSES", row, nestedForm) { }

		public Wd_expenses_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("expense", id);
			Model = Models.Expense.Find(id, userContext, "FWD_EXPENSES", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Expense model = new Models.Expense(userContext) { Identifier = "FWD_EXPENSES" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FWD_EXPENSES");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Expense m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Expense) to ViewModel (Wd_expenses) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Expense) to ViewModel (Wd_expenses) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Expense m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Wd_expenses) to Model (Expense) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Wd_expenses) to Model (Expense) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "expense.codexpense":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Wd_expenses) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Wd_expenses)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Expense.Find(id ?? Navigation.GetStrValue("expense"), m_userContext, "FWD_EXPENSES"); }
			finally { Model ??= new Models.Expense(m_userContext) { Identifier = "FWD_EXPENSES" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Expense.Find(Navigation.GetStrValue("expense"), m_userContext, "FWD_EXPENSES");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Expense(m_userContext) { Identifier = "FWD_EXPENSES" };
...
// USE /[MANUAL MNT VIEWMODEL_LOADPARTIAL WD_EXPENSES]/
		}

// USE /[MANUAL MNT VIEWMODEL_NEW WD_EXPENSES]/
...
// USE /[MANUAL MNT VIEWMODEL_SAVE WD_EXPENSES]/
...
// USE /[MANUAL MNT VIEWMODEL_APPLY WD_EXPENSES]/

// USE /[MANUAL MNT VIEWMODEL_DUPLICATE WD_EXPENSES]/

// USE /[MANUAL MNT VIEWMODEL_DESTROY WD_EXPENSES]/
		public override void Destroy(string id)
		{
			Model = Models.Expense.Find(id, m_userContext, "FWD_EXPENSES");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL MNT VIEWMODEL_CUSTOM WD_EXPENSES]/

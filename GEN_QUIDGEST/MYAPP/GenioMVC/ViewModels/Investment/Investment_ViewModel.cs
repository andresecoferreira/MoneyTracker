...
{
	public class Investment_ViewModel : FormViewModel<Models.Investment>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Investment_ViewModel() : base(null!) { }

		public Investment_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FINVESTMENT", nestedForm) { }

		public Investment_ViewModel(UserContext userContext, Models.Investment row, bool nestedForm = false) : base(userContext, "FINVESTMENT", row, nestedForm) { }

		public Investment_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("investment", id);
			Model = Models.Investment.Find(id, userContext, "FINVESTMENT", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Investment model = new Models.Investment(userContext) { Identifier = "FINVESTMENT" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FINVESTMENT");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Investment m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Investment) to ViewModel (Investment) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				ValCodinvestment = ViewModelConversion.ToString(m.ValCodinvestment);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("Map Model (Investment) to ViewModel (Investment) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Investment m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Investment) to Model (Investment) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				{
					m.ValInvestment_id = ViewModelConversion.ToNumeric(ValInvestment_id);
				}
				...
				m.ValCodinvestment = ViewModelConversion.ToString(ValCodinvestment);
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Investment) to Model (Investment) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "investment.member_id":
						...
						break;
					case "investment.source_id":
						this.ValSource_id = ViewModelConversion.ToString(_value);
						break;
					case "investment.investment_id":
						this.ValInvestment_id = ViewModelConversion.ToNumeric(_value);
						break;
					...
					case "investment.date":
						...
						break;
					case "investment.description":
						this.ValDescription = ViewModelConversion.ToString(_value);
						break;
					case "investment.codinvestment":
						this.ValCodinvestment = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Investment) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Investment)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Investment.Find(id ?? Navigation.GetStrValue("investment"), m_userContext, "FINVESTMENT"); }
			finally { Model ??= new Models.Investment(m_userContext) { Identifier = "FINVESTMENT" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Investment.Find(Navigation.GetStrValue("investment"), m_userContext, "FINVESTMENT");
			}
			finally
			{
				if (Model == null)
		...
		protected override void LoadDocumentsProperties(Models.Investment row)
		{
		}
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Investment(m_userContext) { Identifier = "FINVESTMENT" };
...
// USE /[MANUAL MNT VIEWMODEL_LOADPARTIAL INVESTMENT]/
		}

// USE /[MANUAL MNT VIEWMODEL_NEW INVESTMENT]/
...
// USE /[MANUAL MNT VIEWMODEL_SAVE INVESTMENT]/
...
// USE /[MANUAL MNT VIEWMODEL_APPLY INVESTMENT]/

// USE /[MANUAL MNT VIEWMODEL_DUPLICATE INVESTMENT]/

// USE /[MANUAL MNT VIEWMODEL_DESTROY INVESTMENT]/
		public override void Destroy(string id)
		{
			Model = Models.Investment.Find(id, m_userContext, "FINVESTMENT");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL MNT OVERRQ INVESTMENT_CATEGORY_TYPENAME]/
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Investment__category__name(NameValueCollection qs, bool lazyLoad = false)
		{
			bool investment__category__nameDoLoad = true;
			CriteriaSet investment__category__nameConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("category", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					investment__category__nameConds.Equal(CSGenioAcategory.FldCodcategory, hValue);
			...
			if (investment__category__nameDoLoad)
			{
				...
				investment__category__nameConds.SubSet(search_filters);
...
// USE /[MANUAL MNT OVERRQ INVESTMENT_CATEGORYNAME]/
				...
				*/
				if (Navigation.checkFormMode("category", FormMode.New) || Navigation.checkFormMode("category", FormMode.Duplicate))
					investment__category__nameConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAcategory.FldZzstate, 0)
						.Equal(CSGenioAcategory.FldCodcategory, Navigation.GetStrValue("category")));
				else
					investment__category__nameConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAcategory.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("category", "name");
				ListingMVC<CSGenioAcategory> listing = Models.ModelBase.Where<CSGenioAcategory>(m_userContext, false, investment__category__nameConds, fields, offset, numberItems, sorts, "LED_INVESTMENT__CATEGORY__NAME", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableCategoryName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Category(m_userContext, r, true, _fieldsToSerialize_INVESTMENT__CATEGORY__NAME));
				...
				FillDependant_InvestmentTableCategoryName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_InvestmentTableCategoryName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_InvestmentTableCategoryName(bool lazyLoad = false)
		{
			var row = GetDependant_InvestmentTableCategoryName(this.ValCategory_id);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_INVESTMENT__CATEGORY__NAME = ["Category", "Category.ValCodcategory", "Category.ValZzstate", "Category.ValName"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Investment__member__name(NameValueCollection qs, bool lazyLoad = false)
		{
			bool investment__member__nameDoLoad = true;
			CriteriaSet investment__member__nameConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("member", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					investment__member__nameConds.Equal(CSGenioAmember.FldCodmember, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_InvestmentTableMemberName(lazyLoad);
				return;
			}

			if (investment__member__nameDoLoad)
			{
				List<ColumnSort> sorts = [];
				...
				investment__member__nameConds.SubSet(search_filters);
...
// USE /[MANUAL MNT OVERRQ INVESTMENT_MEMBERNAME]/
				...
				*/
				if (Navigation.checkFormMode("member", FormMode.New) || Navigation.checkFormMode("member", FormMode.Duplicate))
					investment__member__nameConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAmember.FldZzstate, 0)
						.Equal(CSGenioAmember.FldCodmember, Navigation.GetStrValue("member")));
				else
					investment__member__nameConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAmember.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("member", "name");
				ListingMVC<CSGenioAmember> listing = Models.ModelBase.Where<CSGenioAmember>(m_userContext, false, investment__member__nameConds, fields, offset, numberItems, sorts, "LED_INVESTMENT__MEMBER__NAME", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableMemberName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Member(m_userContext, r, true, _fieldsToSerialize_INVESTMENT__MEMBER__NAME));
				...
				FillDependant_InvestmentTableMemberName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_InvestmentTableMemberName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_InvestmentTableMemberName(bool lazyLoad = false)
		{
			var row = GetDependant_InvestmentTableMemberName(this.ValMember_id);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_INVESTMENT__MEMBER__NAME = ["Member", "Member.ValCodmember", "Member.ValZzstate", "Member.ValName"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Investment__source__title(NameValueCollection qs, bool lazyLoad = false)
		{
			bool investment__source__titleDoLoad = true;
			CriteriaSet investment__source__titleConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("source", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					investment__source__titleConds.Equal(CSGenioAsource.FldCodsource, hValue);
			...
			// Area limit
			investment__source__titleDoLoad &= AddCriteriaAreaLimit(investment__source__titleConds, CSGenio.business.CSGenioAmember.FldCodmember, "member", this.ValMember_id, true);
			...
			if (lazyLoad)
			{
				...
				FillDependant_InvestmentTableSourceTitle(lazyLoad);
				return;
			}

			if (string.IsNullOrEmpty(this.ValMember_id))
				investment__source__titleDoLoad = false;

			if (investment__source__titleDoLoad)
			{
				List<ColumnSort> sorts = [];
				...
				investment__source__titleConds.SubSet(search_filters);
...
// USE /[MANUAL MNT OVERRQ INVESTMENT_SOURCETITLE]/
				...
				*/
				if (Navigation.checkFormMode("source", FormMode.New) || Navigation.checkFormMode("source", FormMode.Duplicate))
					investment__source__titleConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAsource.FldZzstate, 0)
						.Equal(CSGenioAsource.FldCodsource, Navigation.GetStrValue("source")));
				else
					investment__source__titleConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAsource.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("source", "title");
				ListingMVC<CSGenioAsource> listing = Models.ModelBase.Where<CSGenioAsource>(m_userContext, false, investment__source__titleConds, fields, offset, numberItems, sorts, "LED_INVESTMENT__SOURCE__TITLE", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableSourceTitle.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Source(m_userContext, r, true, _fieldsToSerialize_INVESTMENT__SOURCE__TITLE));
				...
				FillDependant_InvestmentTableSourceTitle();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_InvestmentTableSourceTitle(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_InvestmentTableSourceTitle(bool lazyLoad = false)
		{
			var row = GetDependant_InvestmentTableSourceTitle(this.ValSource_id);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_INVESTMENT__SOURCE__TITLE = ["Source", "Source.ValCodsource", "Source.ValZzstate", "Source.ValTitle"];
		...
		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				...
				"investment.value" => ViewModelConversion.ToNumeric(modelValue),
				"investment.date" => ViewModelConversion.ToDateTime(modelValue),
				"investment.description" => ViewModelConversion.ToString(modelValue),
				"investment.created_by" => ViewModelConversion.ToString(modelValue),
				"investment.created_at" => ViewModelConversion.ToDateTime(modelValue),
...
// USE /[MANUAL MNT VIEWMODEL_CUSTOM INVESTMENT]/

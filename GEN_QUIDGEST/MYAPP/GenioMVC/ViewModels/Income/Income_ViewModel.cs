...
{
	public class Income_ViewModel : FormViewModel<Models.Income>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		/// Title: "ID" | Type: "N"
		/// </summary>
		public decimal? ValIncome_id { get; set; }
		/// <summary>
		...
		public Income_ViewModel() : base(null!) { }

		public Income_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FINCOME", nestedForm) { }

		public Income_ViewModel(UserContext userContext, Models.Income row, bool nestedForm = false) : base(userContext, "FINCOME", row, nestedForm) { }

		public Income_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("income", id);
			Model = Models.Income.Find(id, userContext, "FINCOME", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Income model = new Models.Income(userContext) { Identifier = "FINCOME" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FINCOME");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Income m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Income) to ViewModel (Income) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				ValCodincome = ViewModelConversion.ToString(m.ValCodincome);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("Map Model (Income) to ViewModel (Income) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Income m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Income) to Model (Income) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				{
					m.ValIncome_id = ViewModelConversion.ToNumeric(ValIncome_id);
				}
				...
				m.ValCodincome = ViewModelConversion.ToString(ValCodincome);
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Income) to Model (Income) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "income.member_id":
						...
						break;
					case "income.source_id":
						this.ValSource_id = ViewModelConversion.ToString(_value);
						break;
					case "income.income_id":
						this.ValIncome_id = ViewModelConversion.ToNumeric(_value);
						break;
					case "income.value":
					...
					case "income.description":
						...
						break;
					case "income.codincome":
						this.ValCodincome = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Income) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Income)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Income.Find(id ?? Navigation.GetStrValue("income"), m_userContext, "FINCOME"); }
			finally { Model ??= new Models.Income(m_userContext) { Identifier = "FINCOME" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Income.Find(Navigation.GetStrValue("income"), m_userContext, "FINCOME");
			}
			finally
			{
				if (Model == null)
		...
		protected override void LoadDocumentsProperties(Models.Income row)
		{
		}
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Income(m_userContext) { Identifier = "FINCOME" };
			...
			Load_Income__category_type__name(qs, lazyLoad);
			Load_Income__category__name(qs, lazyLoad);
			Load_Income__member__name(qs, lazyLoad);
			Load_Income__source__title(qs, lazyLoad);

// USE /[MANUAL MNT VIEWMODEL_LOADPARTIAL INCOME]/
		}

// USE /[MANUAL MNT VIEWMODEL_NEW INCOME]/
...
// USE /[MANUAL MNT VIEWMODEL_SAVE INCOME]/
...
// USE /[MANUAL MNT VIEWMODEL_APPLY INCOME]/

// USE /[MANUAL MNT VIEWMODEL_DUPLICATE INCOME]/

// USE /[MANUAL MNT VIEWMODEL_DESTROY INCOME]/
		public override void Destroy(string id)
		{
			Model = Models.Income.Find(id, m_userContext, "FINCOME");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Income__category_type__name(NameValueCollection qs, bool lazyLoad = false)
		{
			bool income__category_type__nameDoLoad = true;
			CriteriaSet income__category_type__nameConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("category_type", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					income__category_type__nameConds.Equal(CSGenioAcategory_type.FldCodcategory_type, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_IncomeTableCategory_typeName(lazyLoad);
				return;
			}

			if (income__category_type__nameDoLoad)
			{
				...
				income__category_type__nameConds.SubSet(search_filters);
...
// USE /[MANUAL MNT OVERRQ INCOME_CATEGORY_TYPENAME]/
				...
				*/
				if (Navigation.checkFormMode("category_type", FormMode.New) || Navigation.checkFormMode("category_type", FormMode.Duplicate))
					income__category_type__nameConds.SubSet(CriteriaSet.Or()
					...
					income__category_type__nameConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAcategory_type.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("category_type", "name");
				ListingMVC<CSGenioAcategory_type> listing = Models.ModelBase.Where<CSGenioAcategory_type>(m_userContext, false, income__category_type__nameConds, fields, offset, numberItems, sorts, "LED_INCOME__CATEGORY_TYPE__NAME", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableCategory_typeName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Category_type(m_userContext, r, true, _fieldsToSerialize_INCOME__CATEGORY_TYPE__NAME));
				...
				FillDependant_IncomeTableCategory_typeName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_IncomeTableCategory_typeName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_IncomeTableCategory_typeName(bool lazyLoad = false)
		{
			var row = GetDependant_IncomeTableCategory_typeName(this.ValType_id);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_INCOME__CATEGORY_TYPE__NAME = ["Category_type", "Category_type.ValCodcategory_type", "Category_type.ValZzstate", "Category_type.ValName"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Income__category__name(NameValueCollection qs, bool lazyLoad = false)
		{
			bool income__category__nameDoLoad = true;
			CriteriaSet income__category__nameConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("category", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					income__category__nameConds.Equal(CSGenioAcategory.FldCodcategory, hValue);
			...
			// Area limit
			income__category__nameDoLoad &= AddCriteriaAreaLimit(income__category__nameConds, CSGenio.business.CSGenioAcategory_type.FldCodcategory_type, "category_type", this.ValType_id, true);
			...
			if (lazyLoad)
			{
				...
				FillDependant_IncomeTableCategoryName(lazyLoad);
				return;
			}

			if (string.IsNullOrEmpty(this.ValType_id))
				income__category__nameDoLoad = false;

			if (income__category__nameDoLoad)
			{
				List<ColumnSort> sorts = [];
				...
				income__category__nameConds.SubSet(search_filters);
...
// USE /[MANUAL MNT OVERRQ INCOME_CATEGORYNAME]/
				...
				*/
				if (Navigation.checkFormMode("category", FormMode.New) || Navigation.checkFormMode("category", FormMode.Duplicate))
					income__category__nameConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAcategory.FldZzstate, 0)
						.Equal(CSGenioAcategory.FldCodcategory, Navigation.GetStrValue("category")));
				else
					income__category__nameConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAcategory.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("category", "name");
				ListingMVC<CSGenioAcategory> listing = Models.ModelBase.Where<CSGenioAcategory>(m_userContext, false, income__category__nameConds, fields, offset, numberItems, sorts, "LED_INCOME__CATEGORY__NAME", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableCategoryName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Category(m_userContext, r, true, _fieldsToSerialize_INCOME__CATEGORY__NAME));
				...
				FillDependant_IncomeTableCategoryName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_IncomeTableCategoryName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_IncomeTableCategoryName(bool lazyLoad = false)
		{
			var row = GetDependant_IncomeTableCategoryName(this.ValCategory_id);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_INCOME__CATEGORY__NAME = ["Category", "Category.ValCodcategory", "Category.ValZzstate", "Category.ValName"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Income__member__name(NameValueCollection qs, bool lazyLoad = false)
		{
			bool income__member__nameDoLoad = true;
			CriteriaSet income__member__nameConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("member", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					income__member__nameConds.Equal(CSGenioAmember.FldCodmember, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_IncomeTableMemberName(lazyLoad);
				return;
			}

			if (income__member__nameDoLoad)
			{
				List<ColumnSort> sorts = [];
				...
				income__member__nameConds.SubSet(search_filters);
...
// USE /[MANUAL MNT OVERRQ INCOME_MEMBERNAME]/
				...
				*/
				if (Navigation.checkFormMode("member", FormMode.New) || Navigation.checkFormMode("member", FormMode.Duplicate))
					income__member__nameConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAmember.FldZzstate, 0)
						.Equal(CSGenioAmember.FldCodmember, Navigation.GetStrValue("member")));
				else
					income__member__nameConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAmember.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("member", "name");
				ListingMVC<CSGenioAmember> listing = Models.ModelBase.Where<CSGenioAmember>(m_userContext, false, income__member__nameConds, fields, offset, numberItems, sorts, "LED_INCOME__MEMBER__NAME", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableMemberName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Member(m_userContext, r, true, _fieldsToSerialize_INCOME__MEMBER__NAME));
				...
				FillDependant_IncomeTableMemberName();
			}
		}
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_IncomeTableMemberName(bool lazyLoad = false)
		{
		...
		private readonly string[] _fieldsToSerialize_INCOME__MEMBER__NAME = ["Member", "Member.ValCodmember", "Member.ValZzstate", "Member.ValName"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Income__source__title(NameValueCollection qs, bool lazyLoad = false)
		{
			bool income__source__titleDoLoad = true;
			CriteriaSet income__source__titleConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("source", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					income__source__titleConds.Equal(CSGenioAsource.FldCodsource, hValue);
			...
			if (income__source__titleDoLoad)
			{
				...
				income__source__titleConds.SubSet(search_filters);
...
// USE /[MANUAL MNT OVERRQ INCOME_SOURCETITLE]/
				...
				*/
				if (Navigation.checkFormMode("source", FormMode.New) || Navigation.checkFormMode("source", FormMode.Duplicate))
					income__source__titleConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAsource.FldZzstate, 0)
						.Equal(CSGenioAsource.FldCodsource, Navigation.GetStrValue("source")));
				else
					income__source__titleConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAsource.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("source", "title");
				ListingMVC<CSGenioAsource> listing = Models.ModelBase.Where<CSGenioAsource>(m_userContext, false, income__source__titleConds, fields, offset, numberItems, sorts, "LED_INCOME__SOURCE__TITLE", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableSourceTitle.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Source(m_userContext, r, true, _fieldsToSerialize_INCOME__SOURCE__TITLE));
				...
				FillDependant_IncomeTableSourceTitle();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_IncomeTableSourceTitle(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_IncomeTableSourceTitle(bool lazyLoad = false)
		{
			var row = GetDependant_IncomeTableSourceTitle(this.ValSource_id);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_INCOME__SOURCE__TITLE = ["Source", "Source.ValCodsource", "Source.ValZzstate", "Source.ValTitle"];
		...
		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				...
				"income.created_by" => ViewModelConversion.ToString(modelValue),
				"income.created_at" => ViewModelConversion.ToDateTime(modelValue),
				"income.updated_by" => ViewModelConversion.ToString(modelValue),
				"income.updated_at" => ViewModelConversion.ToDateTime(modelValue),
				"income.codincome" => ViewModelConversion.ToString(modelValue),
...
// USE /[MANUAL MNT VIEWMODEL_CUSTOM INCOME]/

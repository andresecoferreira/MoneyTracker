...
{
	public class Income_CategoryValName_ViewModel : MenuListViewModel<Models.Category>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Income_CategoryValName_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Income_CategoryValName";
		...
		{
// USE /[MANUAL MNT LIST_LIMITS INCOME_CATEGORYNAME]/
		...
		public Income_CategoryValName_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Income_CategoryValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Income_CategoryValName_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Income_CategoryValName_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Category.AddEPH<CSGenioAcategory>(ref u, crs, "IBL_INCOME__CATEGORY__NAME");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Category.AddEPH<CSGenioAcategory>(ref u, null, "IBL_INCOME__CATEGORY__NAME"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcategory> Qlisting, ref CriteriaSet conditions)
		{
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_INCOME__CATEGORY__NAME");
			...
			conditions.SubSets.Add(income__category__nameConds);
			income__category__nameConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL MNT OVERRQ INCOME_CATEGORYNAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAcategory>(m_userContext, false, ref income__category__nameConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_INCOME__CATEGORY__NAME", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL MNT OVERRQLSTEXP INCOME_CATEGORYNAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL MNT OVERRQLIST INCOME_CATEGORYNAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcategory.GetInformation(), QMVC_POS_RECORD, sorts, income__category__nameConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAcategory> listing = Models.ModelBase.Where<CSGenioAcategory>(m_userContext, distinct, income__category__nameConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_INCOME__CATEGORY__NAME", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapIncome_CategoryValName(listing);

				Menu.Identifier = "IBL_INCOME__CATEGORY__NAME";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_INCOME__CATEGORY__NAME";
		...
		private List<Income_CategoryValName_RowViewModel> MapIncome_CategoryValName(ListingMVC<CSGenioAcategory> Qlisting)
		{
			List<Income_CategoryValName_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapIncome_CategoryValName(row));
					i++;
				}
			}
		...
		/// to a Income_CategoryValName_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Income_CategoryValName_RowViewModel MapIncome_CategoryValName(CSGenioAcategory row)
		{
			var model = new Income_CategoryValName_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL MNT VIEWMODEL_CUSTOM INCOME_CATEGORYVALNAME]/

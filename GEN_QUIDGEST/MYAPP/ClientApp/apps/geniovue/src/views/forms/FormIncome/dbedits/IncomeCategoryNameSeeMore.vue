<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-income-category-name-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['INCOME__CATEGORY__NAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'IncomeCategoryNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('INCOME__CATEGORY__NAME_SeeMore', false),

				interfaceMetadata: {
					id: 'INCOME__CATEGORY__NAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'INCOME__CATEGORY__NAME_SeeMore',
					name: 'INCOME__CATEGORY__NAME_SeeMore',
					controller: 'INCOME',
					action: 'INCOME_CategoryValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-income-category-name',
				dismissAction: this.close,
				returnElement: 'INCOME__CATEGORY__NAME_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-income-category-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Income_Income_CategoryValName',

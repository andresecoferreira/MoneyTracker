<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-investment-category-name-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['INVESTMENT__CATEGORY__NAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'InvestmentCategoryNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('INVESTMENT__CATEGORY__NAME_SeeMore', false),

				interfaceMetadata: {
					id: 'INVESTMENT__CATEGORY__NAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'INVESTMENT__CATEGORY__NAME_SeeMore',
					name: 'INVESTMENT__CATEGORY__NAME_SeeMore',
					controller: 'INVESTMENT',
					action: 'INVESTMENT_CategoryValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-investment-category-name',
				dismissAction: this.close,
				returnElement: 'INVESTMENT__CATEGORY__NAME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.CATEGORIES06648),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-investment-category-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Investment_CategoryValName',
							serverMode: true,
						...
						uuid: 'Investment_Investment_CategoryValName',

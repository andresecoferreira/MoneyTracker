<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-month-year-year-number-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['MONTH__YEAR__YEAR_NUMBER_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'MonthYearYearNumberSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('MONTH__YEAR__YEAR_NUMBER_SeeMore', false),

				interfaceMetadata: {
					id: 'MONTH__YEAR__YEAR_NUMBER_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'MONTH__YEAR__YEAR_NUMBER_SeeMore',
					name: 'MONTH__YEAR__YEAR_NUMBER_SeeMore',
					controller: 'MONTH',
					action: 'MONTH_YearValYear_number'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-month-year-year-number',
				dismissAction: this.close,
				returnElement: 'MONTH__YEAR__YEAR_NUMBER_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.YEARS39740),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-month-year-year-number')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Month_YearValYear_number',
							serverMode: true,
							pkColumn: 'ValCodyear',
							tableAlias: 'YEAR',
							tableNamePlural: computed(() => this.Resources.YEARS39740),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
							...
							defaultSearchColumnName: 'ValYear_number',
							defaultSearchColumnNameOriginal: 'ValYear_number',
						...
						uuid: 'Month_Month_YearValYear_number',

<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-expense-category-name-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['EXPENSE__CATEGORY__NAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'ExpenseCategoryNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('EXPENSE__CATEGORY__NAME_SeeMore', false),

				interfaceMetadata: {
					id: 'EXPENSE__CATEGORY__NAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'EXPENSE__CATEGORY__NAME_SeeMore',
					name: 'EXPENSE__CATEGORY__NAME_SeeMore',
					controller: 'EXPENSE',
					action: 'EXPENSE_CategoryValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-expense-category-name',
				dismissAction: this.close,
				returnElement: 'EXPENSE__CATEGORY__NAME_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-expense-category-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Expense_Expense_CategoryValName',

<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-income-member-name-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['INCOME__MEMBER__NAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'IncomeMemberNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('INCOME__MEMBER__NAME_SeeMore', false),

				interfaceMetadata: {
					id: 'INCOME__MEMBER__NAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'INCOME__MEMBER__NAME_SeeMore',
					name: 'INCOME__MEMBER__NAME_SeeMore',
					controller: 'INCOME',
					action: 'INCOME_MemberValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-income-member-name',
				dismissAction: this.close,
				returnElement: 'INCOME__MEMBER__NAME_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-income-member-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Income_Income_MemberValName',

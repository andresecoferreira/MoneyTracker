<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-income-source-title-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['INCOME__SOURCE__TITLE_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'IncomeSourceTitleSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('INCOME__SOURCE__TITLE_SeeMore', false),

				interfaceMetadata: {
					id: 'INCOME__SOURCE__TITLE_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'INCOME__SOURCE__TITLE_SeeMore',
					name: 'INCOME__SOURCE__TITLE_SeeMore',
					controller: 'INCOME',
					action: 'INCOME_SourceValTitle'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-income-source-title',
				dismissAction: this.close,
				returnElement: 'INCOME__SOURCE__TITLE_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-income-source-title')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Income_Income_SourceValTitle',

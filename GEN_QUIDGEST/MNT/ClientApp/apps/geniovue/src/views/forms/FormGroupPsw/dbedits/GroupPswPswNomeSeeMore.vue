<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-group-psw-psw-nome-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['GROUP_PSW__PSW__NOME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'GroupPswPswNomeSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('GROUP_PSW__PSW__NOME_SeeMore', false),

				interfaceMetadata: {
					id: 'GROUP_PSW__PSW__NOME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'GROUP_PSW__PSW__NOME_SeeMore',
					name: 'GROUP_PSW__PSW__NOME_SeeMore',
					controller: 'GROUP_PSW',
					action: 'GROUP_PSW_PswValNome'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-group-psw-psw-nome',
				dismissAction: this.close,
				returnElement: 'GROUP_PSW__PSW__NOME_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-group-psw-psw-nome')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Group_psw_Group_psw_PswValNome',

<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-member-groupname-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['MEMBER__GROUPNAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'MemberGroupnameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('MEMBER__GROUPNAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'MEMBER__GROUPNAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'MEMBER__GROUPNAME_____SeeMore',
					name: 'MEMBER__GROUPNAME_____SeeMore',
					controller: 'MEMBER',
					action: 'MEMBER_GroupValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-member-groupname',
				dismissAction: this.close,
				returnElement: 'MEMBER__GROUPNAME_____see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-member-groupname')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Member_Member_GroupValName',

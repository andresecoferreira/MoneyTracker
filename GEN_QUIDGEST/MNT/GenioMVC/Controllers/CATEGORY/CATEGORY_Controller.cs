...
namespace GenioMVC.Controllers
{
	public partial class CategoryController : ControllerBase
	{
		...
		public CategoryController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION CATEGORY]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAcategory>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER CATEGORY]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Category row = new Models.Category(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("category");
			...
			try
			{
				...
				{
					case "CATEGORY__CATEGORY_TYPE__NAME":	// Field (DB)
						{
							var model = new Category_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Category__category_type__name(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "CATEGORY__CATEGORY_TYPE__NAME":	// Field (DB)
						values = new Category_ViewModel(UserContext.Current).GetDependant_CategoryTableCategory_typeName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Category" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Category([FromBody]Category_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "category",
				(primaryKey) => Models.Category.Find(primaryKey, UserContext.Current, "FCATEGORY"),
				(model) => formData.MapToModel(model as Models.Category)
			);
		}
		...
		{
			return base.GetDocumsTickets("CATEGORY", requestModel.FieldName, requestModel.KeyValue);
		}

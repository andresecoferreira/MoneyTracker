...
namespace GenioMVC.Controllers
{
	public partial class SourceController : ControllerBase
	{
		...
		public SourceController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION SOURCE]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAsource>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER SOURCE]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Source row = new Models.Source(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("source");
			...
			try
			{
				...
				{
					case "SOURCE__MEMBER__NAME":	// Field (DB)
						{
							var model = new Source_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Source__member__name(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "SOURCE__MEMBER__NAME":	// Field (DB)
						values = new Source_ViewModel(UserContext.Current).GetDependant_SourceTableMemberName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Source" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Source([FromBody]Source_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "source",
				(primaryKey) => Models.Source.Find(primaryKey, UserContext.Current, "FSOURCE"),
				(model) => formData.MapToModel(model as Models.Source)
			);
		}
		...
		{
			return base.GetDocumsTickets("SOURCE", requestModel.FieldName, requestModel.KeyValue);
		}

...
namespace GenioMVC.Controllers
{
	public partial class MemberController : ControllerBase
	{
		...
		public MemberController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION MEMBER]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAmember>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER MEMBER]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Member row = new Models.Member(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("member");
			...
			try
			{
				...
				{
					case "MEMBER__GROUPNAME____":	// Field (DB)
						{
							var model = new Member_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Member__groupname____(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "MEMBER__GROUPNAME____":	// Field (DB)
						values = new Member_ViewModel(UserContext.Current).GetDependant_MemberTableGroupName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Member" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Member([FromBody]Member_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "member",
				(primaryKey) => Models.Member.Find(primaryKey, UserContext.Current, "FMEMBER"),
				(model) => formData.MapToModel(model as Models.Member)
			);
		}
		...
		{
			return base.GetDocumsTickets("MEMBER", requestModel.FieldName, requestModel.KeyValue);
		}

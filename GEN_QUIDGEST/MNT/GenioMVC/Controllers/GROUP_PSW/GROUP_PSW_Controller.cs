...
namespace GenioMVC.Controllers
{
	public partial class Group_pswController : ControllerBase
	{
		...
		public Group_pswController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL MNT CONTROLLER_NAVIGATION GROUP_PSW]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAgroup_psw>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL MNT MANUAL_CONTROLLER GROUP_PSW]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Group_psw row = new Models.Group_psw(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("group_psw");
			...
			try
			{
				...
				{
					case "GROUP_PSW__GROUP__NAME":	// Field (DB)
						{
							var model = new Group_psw_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Group_psw__group__name(qs);
							result = model.TableGroupName;
						}
						break;
					case "GROUP_PSW__PSW__NOME":	// Field (DB)
						{
							var model = new Group_psw_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Group_psw__psw__nome(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "GROUP_PSW__GROUP__NAME":	// Field (DB)
						values = new Group_psw_ViewModel(UserContext.Current).GetDependant_Group_pswTableGroupName(Selected);
						break;
					case "GROUP_PSW__PSW__NOME":	// Field (DB)
						values = new Group_psw_ViewModel(UserContext.Current).GetDependant_Group_pswTablePswNome(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Group_psw" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Group_psw([FromBody]Group_psw_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "group_psw",
				(primaryKey) => Models.Group_psw.Find(primaryKey, UserContext.Current, "FGROUP_PSW"),
				(model) => formData.MapToModel(model as Models.Group_psw)
			);
		}
		...
		{
			return base.GetDocumsTickets("GROUP_PSW", requestModel.FieldName, requestModel.KeyValue);
		}

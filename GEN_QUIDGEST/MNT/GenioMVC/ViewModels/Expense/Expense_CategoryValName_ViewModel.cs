...
{
	public class Expense_CategoryValName_ViewModel : MenuListViewModel<Models.Category>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Expense_CategoryValName_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Expense_CategoryValName";
		...
		{
// USE /[MANUAL MNT LIST_LIMITS EXPENSE_CATEGORYNAME]/
		...
		public Expense_CategoryValName_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Expense_CategoryValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Expense_CategoryValName_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Expense_CategoryValName_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Expense_CategoryValName_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Category.AddEPH<CSGenioAcategory>(ref u, crs, "IBL_EXPENSE__CATEGORY__NAME");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Category.AddEPH<CSGenioAcategory>(ref u, null, "IBL_EXPENSE__CATEGORY__NAME"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcategory> Qlisting, ref CriteriaSet conditions)
		{
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_EXPENSE__CATEGORY__NAME");
			...
			conditions.SubSets.Add(expense__category__nameConds);
			expense__category__nameConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL MNT OVERRQ EXPENSE_CATEGORYNAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAcategory>(m_userContext, false, ref expense__category__nameConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EXPENSE__CATEGORY__NAME", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL MNT OVERRQLSTEXP EXPENSE_CATEGORYNAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL MNT OVERRQLIST EXPENSE_CATEGORYNAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcategory.GetInformation(), QMVC_POS_RECORD, sorts, expense__category__nameConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAcategory> listing = Models.ModelBase.Where<CSGenioAcategory>(m_userContext, distinct, expense__category__nameConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EXPENSE__CATEGORY__NAME", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapExpense_CategoryValName(listing);

				Menu.Identifier = "IBL_EXPENSE__CATEGORY__NAME";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_EXPENSE__CATEGORY__NAME";
		...
		private List<Expense_CategoryValName_RowViewModel> MapExpense_CategoryValName(ListingMVC<CSGenioAcategory> Qlisting)
		{
			List<Expense_CategoryValName_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapExpense_CategoryValName(row));
					i++;
				}
			}
		...
		/// to a Expense_CategoryValName_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Expense_CategoryValName_RowViewModel MapExpense_CategoryValName(CSGenioAcategory row)
		{
			var model = new Expense_CategoryValName_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL MNT VIEWMODEL_CUSTOM EXPENSE_CATEGORYVALNAME]/
